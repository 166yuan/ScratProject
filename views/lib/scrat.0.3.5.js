(function (global) {
    ...
    proto.async = function (names, onload) {
        ...
        var reactor = new scrat.Reactor(names, function (err) {
            var args = [];
            each(names, function (id) {
                args.push(err === null ? null : require(id));
            });
    ...
    proto.load = function (url, options) {
        ...
        node.onerror = function onerror() {
            clearTimeout(tid);
            clearInterval(intId);
            if (options.onload) options.onload.call(scrat,null);
    ...
    rproto.push = function () {
        ...
        function onload(err) {
            if (--that.length === 0) that.callback(err);
        }
    ...
    rproto.run = function () {
        ...
        if (combo) {
            ...
            each((depends.css || []).concat(depends.js || []), function (res) {
                scrat.load(that.genUrl(res.id), function (err) {
                    res.loaded = !!err;
                    while (res.onload.length) {
                        var onload = res.onload.shift();
                        onload.call(res,err);
    ...
    rproto.genUrl = function (ids) {
        ...
        return url + (~url.indexOf('?') ? '&' : '?') + options.hash +"&uc_param_str=prdnfrpfbivelabtbmntpvsscpnwos"+(window.location.href.indexOf("gc=233") !== -1?"&gc=233":"");
    };
